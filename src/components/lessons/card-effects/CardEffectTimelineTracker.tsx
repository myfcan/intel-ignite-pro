...
export const CardEffectTimelineTracker: React.FC<CardEffectProps> = ({ isActive = false, duration }) => {
  ...
  const startAnimation = () => {
    ...
    timersRef.current.push(setTimeout(() => { setScene(2); setActiveWeek(1); }, 2000 * scale));
    timersRef.current.push(setTimeout(() => setActiveWeek(2), 3500 * scale));
    timersRef.current.push(setTimeout(() => setActiveWeek(3), 5000 * scale));
    timersRef.current.push(setTimeout(() => setActiveWeek(4), 6500 * scale));
    ...
    <div className="relative w-full min-h-[480px] h-[60vh] max-h-[600px] overflow-hidden rounded-xl bg-gradient-to-br from-slate-900 via-sky-900/30 to-blue-950">
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-start px-6 pt-8 pb-16">
        ...
        <AnimatePresence>
          {scene >= 4 && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 * scale }} className="mt-4">
              <motion.div className="flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-sky-500/20 to-blue-500/20 rounded-xl border border-sky-500/30" animate={{ boxShadow: ['0 0 15px rgba(14,165,233,0.2)', '0 0 30px rgba(14,165,233,0.4)', '0 0 15px rgba(14,165,233,0.2)'] }} transition={{ duration: 2 * scale, repeat: 0 }}>
        ...
        <div className="flex gap-2 mt-auto pt-4">
          {[1, 2, 3, 4].map((s) => (
            <motion.div key={s} className="w-2.5 h-2.5 rounded-full" animate={{ backgroundColor: scene >= s ? '#0ea5e9' : 'rgba(255,255,255,0.2)', scale: scene === s ? 1.3 : 1 }} transition={{ duration: 0.4 * scale }} />
          ))}
        </div>
      </div>

      <motion.div className="absolute top-4 right-4 flex items-center gap-1.5 px-3 py-1.5 bg-sky-500/20 border border-sky-500/30 rounded-full" initial={{ opacity: 0, x: 20 }} animate={{ opacity: scene > 0 ? 1 : 0, x: scene > 0 ? 0 : 20 }} transition={{ duration: 0.5 * scale }}>
