...
export const CardEffectPlaygroundCreator: React.FC<CardEffectProps> = ({ isActive = false }) => {
  ...
  const startAnimation = () => {
    ...
    timersRef.current.push(setTimeout(() => { setScene(3); setShowResponse(true); }, 5500));
  ...
  return (
    <div className="relative w-full min-h-[480px] h-[60vh] max-h-[600px] overflow-hidden rounded-xl bg-gradient-to-br from-slate-900 via-teal-900/30 to-cyan-950">
      <div className="absolute inset-0 opacity-10">
        <div className="font-mono text-[8px] text-teal-400 p-4 leading-relaxed overflow-hidden">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="opacity-30">{'>'} prompt_execute --mode=creative</div>
          ))}
        </div>
      </div>
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-start px-6 pt-8 pb-16">
        ...
        <AnimatePresence>
          {scene >= 1 && (
            ...
            >
              <motion.div
                className="w-16 h-16 rounded-xl bg-teal-500/20 border-2 border-teal-400 flex items-center justify-center"
                animate={{ boxShadow: ['0 0 20px rgba(20,184,166,0.3)', '0 0 40px rgba(20,184,166,0.5)', '0 0 20px rgba(20,184,166,0.3)'] }}
        ...
        <AnimatePresence>
          {scene >= 2 && (
            <motion.div
              className="w-full max-w-sm bg-slate-800/70 border border-teal-400/30 rounded-xl overflow-hidden"
            ...
            >
                ...
                <AnimatePresence>
                  {showResponse && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="bg-teal-500/10 border border-teal-400/30 rounded-lg p-3"
                    >
              ...
              <div className="flex items-center justify-between px-4 py-2 border-t border-teal-400/20 bg-slate-800/50">
                <span className="text-xs text-slate-500">Prompt prático</span>
                <motion.div
                  className="flex items-center gap-1.5 text-teal-400"
                  animate={{ scale: scene >= 2 ? [1, 1.1, 1] : 1 }}
                  transition={{ duration: 0.5, repeat: scene >= 2 && scene < 3 ? Infinity : 0 }}
                >
        ...
        <AnimatePresence>
          {scene >= 4 && (
            ...
            >
              <motion.div
                className="flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-teal-500/20 to-cyan-500/20 rounded-xl border border-teal-500/30"
        ...
        <div className="flex gap-2 mt-auto pt-4">
          {[1, 2, 3, 4].map((s) => (
            <motion.div
              ...
              animate={{ backgroundColor: scene >= s ? '#14b8a6' : 'rgba(255,255,255,0.2)', scale: scene === s ? 1.3 : 1 }}
      ...
      <motion.div
        className="absolute top-4 right-4 flex items-center gap-1.5 px-3 py-1.5 bg-teal-500/20 border border-teal-500/30 rounded-full"
