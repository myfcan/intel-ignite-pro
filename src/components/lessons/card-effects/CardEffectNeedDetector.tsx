...
/**
 * CardEffectNeedDetector - Detector de Necessidades
 * 
 * 4 Cenas progressivas (~12s total, 3s por cena):
 * 1. Scanner aparecendo
 * 2. Escaneando necessidades
 * 3. Lista de necessidades detectadas
 * 4. Conclusão - oportunidades encontradas
 * 
 * Roda 2x automaticamente
 * Estilo visual: fundo marrom quente igual Aula 2
 */
export const CardEffectNeedDetector: React.FC<CardEffectProps> = ({ isActive = false }) => {
  ...
  const startAnimation = () => {
    ...
    // Cena 3: Mostrar necessidades detectadas
    timersRef.current.push(setTimeout(() => {
      setScene(3);
      needs.forEach((_, i) => {
        timersRef.current.push(setTimeout(() => {
  ...
  return (
    <div className="relative w-full min-h-[480px] h-[60vh] max-h-[600px] overflow-hidden rounded-xl bg-gradient-to-br from-amber-950 via-orange-950 to-rose-950">
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-start px-6 pt-8 pb-16">
        ...
        <AnimatePresence>
          {scene === 2 && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
            >
              <p className="text-center text-orange-200 text-sm mb-2">
        ...
        {/* ========== CENA 3: Necessidades detectadas ========== */}
        <AnimatePresence>
          {scene >= 3 && (
            <motion.div
