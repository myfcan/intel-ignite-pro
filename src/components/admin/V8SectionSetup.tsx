...
export function V8SectionSetup({ sections, quizzes, playgrounds, onApply, onBack }: V8SectionSetupProps) {
  ...
  const activeBadges = (cfg: SectionConfig) => {
    ...
    if (cfg.hasImage) badges.push({ label: "Imagem", color: "bg-sky-500/20 text-sky-300" });
    if (cfg.hasQuiz) badges.push({ label: "Quiz", color: "bg-amber-500/20 text-amber-300" });
    if (cfg.hasPlayground) badges.push({ label: "Playground", color: "bg-violet-500/20 text-violet-300" });
    return badges;
  };
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-sm font-semibold text-slate-300">Setup de Seções</h2>
          <p className="text-[10px] text-slate-500 mt-0.5">
            Configure o que cada seção terá antes de gerar o JSON final
          </p>
        </div>
        <span className="text-[10px] text-slate-600">{sections.length} seções</span>
      </div>
                          ...
                          {cfg.hasImage && (
                            ...
                            >
                              <input
                                ...
                                className="w-full mt-2 bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-[11px] text-white focus:outline-none focus:border-sky-500 placeholder:text-slate-600"
                              />
                            </motion.div>
                          )}
                        </AnimatePresence>
      ...
      <div className="flex gap-3 mt-4">
        <button
          onClick={onBack}
          className="px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-xs font-semibold text-slate-400 hover:bg-white/10 transition-colors"
        >
          ← Voltar
        </button>
        <button
